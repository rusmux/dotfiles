...
ENV PATH="$PATH:/nix/var/nix/profiles/default/bin" \
  USER=root
RUN nix run home-manager/master -- switch --flake "github:rusmux/dotfiles?dir=home-manager#$USER-$(nix eval --raw --impure --expr 'builtins.currentSystem')"
...
RUN mkdir -p "$HOME/.config/mise" \
  ...
  && mise install helm kubectl python
